...
async function processScene(scene, options) {
    ...
    */
    ...
    //由切分树节点生成瓦片树节点：仅根与叶子产出瓦片内容。
    //中间层不产出内容（其图元数与叶子接近，空间对半切不会显著减少材质数，
    //近景混合渲染反而增加draw call，且逐层下载解码拖慢首屏），作为纯导航节点强制下钻
    function buildTileNode(splitNode, isRoot) {
        const isLeaf = !splitNode.children || splitNode.children.length === 0;
        const makeContent = isRoot || isLeaf;
        const tile = makeContent ? makeTile() : null;
        if (tile) {
