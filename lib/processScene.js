...
async function processScene(scene, options) {
    ...
    */
    ...
    if (options.split !== 'material') {
        root = buildTileNode(splitSpatially(items, 0));
    } else {
        ...
        root = {
            tile: null,
            children: buckets.filter(bucket => bucket.length > 0).map(bucket => buildTileNode({ items: bucket })),
        };
    }
    ...
    //由切分树节点生成瓦片树节点：内部节点与叶子节点均产出瓦片内容
    function buildTileNode(splitNode) {
        const tile = makeTile();
        ...
        return {
            tile,
            children: (splitNode.children || []).map(buildTileNode),
        };
    }
