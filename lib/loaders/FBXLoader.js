...
class FBXTreeParser {
	...
	parseImage( videoNode ) {
		...
		if ( typeof content === 'string' ) { // ASCII format
			...
			return { name: fileName, uri: 'data:' + type + ';base64,' + content, mimeType: type };

		} else { // Binary Format

			return { name: fileName, data: new Blob( [ content ], { type: type } ), mimeType: type };
	...
	parseTextures( images ) {
		...
		if ( 'Texture' in fbxTree.Objects ) {
			...
			for ( const nodeID in textureNodes ) {
				...
				texture && textureMap.set( parseInt( nodeID ), texture );
	...
	loadTexture( textureNode, images ) {
		...
		if ( children !== undefined && children.length > 0 && images[ children[ 0 ].ID ] !== undefined ) {
			...
			image = images[ children[ 0 ].ID ];

			if ( image.uri && (image.uri.indexOf( 'blob:' ) === 0 || image.uri.indexOf( 'data:' ) === 0) ) {
		...
		if ( image === undefined || image.data === undefined ) {
			...
			console.warn( 'FBXLoader: Illegal image data: ' + image );
			return undefined;
	...
	addGlobalSceneSettings() {
		...
		if ( 'GlobalSettings' in fbxTree ) {
			...
			if ( 'UpAxis' in fbxTree.GlobalSettings ) {

				// FBX UpAxis枚举（实测验证）: 1=Y, 2=Z, 3=X
				const upAxis = fbxTree.GlobalSettings.UpAxis.value;
				sceneGraph.userData.upAxis = { 1: 'Y', 2: 'Z', 3: 'X' }[ upAxis ] || 'Y';
