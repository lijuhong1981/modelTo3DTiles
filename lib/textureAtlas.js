...
// 惰性加载sharp（原生模块,exe环境可能未随附）：仅在需要图像处理时才require
...
function getSharp() {
    if (!sharpModule) {
        try {
            ...
            throw new Error("图像处理模块sharp不可用(--textureAtlas/--resampleTextures需要)。独立exe未内置sharp原生库,请改用npm安装方式运行: npm install -g @lijuhong1981/modelto3dtiles");
        }
    }
    return sharpModule;
};
            ...
            ['map', 'normalMap', 'emissiveMap', 'aoMap', 'specularMap', 'bumpMap',
                ...
                tasks.push((async () => {
                    ...
                    if (width === metadata.width && height === metadata.height)
                        return;
                    const resized = await getSharp()(bytes)
