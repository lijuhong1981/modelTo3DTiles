...
async function processTiles(rootNode, options) {
    ...
    */
    async function writeNode(node) {
        ...
        if (box) {
            ...
            if (!node.tile) {
                ...
                //ge = 包围盒对角线/128，在默认maxSSE=16、900px视口下约于0.38倍对角线距离处refine子层。
                //根内容为全细节几何，多停留于粗层无画质代价；取景距离(~0.75倍对角线)必须安全落在切换点之外，
                //否则全景会一直refine到全部叶子
                json.geometricError = isLeaf ? 0 : box.getSize(new Vector3()).length() / 128;
            }
        }
        return { json, box };
    }
     ...
     * 实例化组代表网格位于原点(几何为局部坐标),不能用Box3.setFromObject,
     * 改用其userData.worldBounds(已含锚点/旋转变换)参与并集。
     * @param {Tile} tile
     * @returns {Box3}
    */
