...
async function writeScene(scene, options, featureNames) {
    ...
    const externalized = externalizeTextures(Buffer.from(gltfBuffer));
    ...
    const glbBuffer = attachFeatureMetadata(externalized.glbBuffer, featureNames);
    const useB3dm = options.b3dm !== false;
    //b3dm规范：内嵌glTF使用EXT_mesh_features时FeatureTable为空（BATCH_LENGTH为0）
    const tileBuffer = useB3dm ? createB3dm(glbBuffer) : glbBuffer;
